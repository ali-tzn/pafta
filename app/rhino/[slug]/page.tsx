...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return guide ? createSeoMetadata({ title: guide.title, description: guide.description, path: `/rhino/${slug}`, keywords: ["Rhino", "Grasshopper", guide.category, ...guide.keyPoints] }) : {};
}
export default async function RhinoGuidePage({ params }: Props) {
  const { slug } = await params; const guide = catalog.guides.find((item) => item.slug === slug);
