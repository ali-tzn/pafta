...
export function generateStaticParams() {
  return guideCollections.map(({ slug }) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const guide = getGuideCollection(slug);
  if (!guide) return {};
  return createSeoMetadata({ title: guide.name, description: guide.description, path: `/rehberler/${guide.slug}`, keywords: guide.keywords });
}
...
export default async function GuidePage({ params }: Props) {
  ...
  const guide = getGuideCollection(slug);
  if (!guide) notFound();
  const index = guideCollections.findIndex((item) => item.slug === slug);
  const related = guideCollections.filter((item) => item.slug !== slug).slice(0, 3);
  const next = guideCollections[(index + 1) % guideCollections.length];

  return (
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-white sm:px-6 sm:py-16">
      <ArticleSeo title={guide.name} description={guide.description} path={`/rehberler/${guide.slug}`} section="Proje Rehberleri" sectionPath="/rehberler" keywords={guide.keywords} />
      <article className="mx-auto max-w-5xl">
        ...
        <ContentMeta items={guide.sections.map((section, sectionIndex) => ({ id: `rehber-bolum-${sectionIndex + 1}`, label: section.title }))} sourceNote="Mimarlık stüdyosu ve proje teslim iş akışı" />
        ...
        <RelatedContent items={related.map((item) => ({ href: `/rehberler/${item.slug}`, title: item.name, description: item.description, label: "Proje rehberi" }))} next={{ href: `/rehberler/${next.slug}`, title: next.name }} />
      </article>
    </main>
  );
}
