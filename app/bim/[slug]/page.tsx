...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return createSeoMetadata({ title: guide.title, description: guide.description, path: `/bim/${guide.slug}`, keywords: [guide.category, ...guide.keyPoints] });
}
export default async function BimGuidePage({ params }: Props) {
  ...
  const index = bimGuides.findIndex((item) => item.slug === slug); const related = bimGuides.filter((item) => item.slug !== slug).sort((a,b) => Number(b.category === guide.category) - Number(a.category === guide.category)).slice(0,3); const next = bimGuides[(index + 1) % bimGuides.length];
  return <main className="min-h-screen bg-slate-950 px-4 py-10 text-white sm:px-6 sm:py-16"><ArticleSeo title={guide.title} description={guide.description} path={`/bim/${guide.slug}`} section="BIM Merkezi" sectionPath="/bim" keywords={[guide.category, ...guide.keyPoints]}/><article className="mx-auto max-w-4xl">
    ...
    <ContentMeta sourceNote="BIM uygulama, BEP ve koordinasyon kaynakları" sources={[{ label: "buildingSMART Standards", href: "https://www.buildingsmart.org/standards/" }, { label: "UK BIM Framework", href: "https://www.ukbimframework.org/" }]} /><TechnicalGuideDetails guide={guide} kind="bim" />
    ...
    <section className="mt-10 rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-6"><h2 className="text-xl font-semibold text-cyan-300">Sonuç</h2><p className="mt-3 leading-7 text-slate-300">BIM’de değer yalnız model üretmekten değil; doğru bilginin doğru zamanda, tanımlı sorumluluk ve kontrol süreciyle paylaşılmasından doğar. Bu rehberi proje BEP’i, sözleşme koşulları ve kurum standardıyla birlikte kullan.</p></section><RelatedContent items={related.map((item) => ({ href: `/bim/${item.slug}`, title: item.title, description: item.description, label: item.category }))} next={{ href: `/bim/${next.slug}`, title: next.title }} />
  </article></main>;
}
