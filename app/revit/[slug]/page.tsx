...
export default async function RevitGuidePage({ params }: Props) {
  ...
  const index = revitGuides.findIndex((item) => item.slug === slug); const related = revitGuides.filter((item) => item.slug !== slug).sort((a,b) => Number(b.category === guide.category) - Number(a.category === guide.category)).slice(0,3); const next = revitGuides[(index + 1) % revitGuides.length];
  return <main className="min-h-screen bg-slate-950 px-4 py-10 text-white sm:px-6 sm:py-16"><ArticleSeo title={guide.title} description={guide.description} path={`/revit/${guide.slug}`} section="Revit Merkezi" sectionPath="/revit" keywords={[guide.category, ...guide.keyPoints]}/><article className="mx-auto max-w-4xl">
    ...
    <ContentMeta sourceNote="Revit iş akışı, proje standardı ve resmî dokümantasyon kontrolü" sources={[{ label: "Autodesk Revit 2026 Help", href: "https://help.autodesk.com/view/RVT/2026/ENU/" }]} /><TechnicalGuideDetails guide={guide} kind="revit" />
    ...
    <Block title="Sık yapılan hatalar ve önleme notları" items={guide.pitfalls.map((item, index) => `${item} — Önlemek için ${guide.workflow[index % guide.workflow.length].toLocaleLowerCase("tr-TR")}.`)} warning/>
    <section className="mt-10 rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-6"><h2 className="text-xl font-semibold text-cyan-300">Son kontrol</h2><p className="mt-3 leading-7 text-slate-300">İşlemi doğrudan teslim modelinde uygulamadan önce proje kopyasında dene. Revit sürümünü, ofis şablonunu, görünüş ayarlarını ve ekip modelleme standardını birlikte kontrol et.</p></section><RelatedContent items={related.map((item) => ({ href: `/revit/${item.slug}`, title: item.title, description: item.description, label: item.category }))} next={{ href: `/revit/${next.slug}`, title: next.title }} />
  </article></main>;
}
