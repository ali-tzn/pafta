...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return guide ? createSeoMetadata({ title: guide.title, description: guide.description, path: `/sketchup/${slug}`, keywords: ["SketchUp", guide.category, ...guide.keyPoints] }) : {};
}
export default async function SketchUpGuidePage({ params }: Props) {
  const { slug } = await params; const guide = catalog.guides.find((item) => item.slug === slug);
